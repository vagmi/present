// Present embeddable widget. Usage on any site:
...
async function mount(el: HTMLElement) {
  if (el.dataset.presentMounted) return;
  el.dataset.presentMounted = "1";

  const id = el.dataset.present;
  if (!id) return;
  ...
  try {
    ...
    console.error("[present] failed to load", id, err);
    root.innerHTML = "";
  }
}
...
function scan() {
  document
    .querySelectorAll<HTMLElement>("[data-present]")
