...
describe("uploads service", () => {
  describe("put", () => {
    it("stores under an org-prefixed key derived from the file extension", async () => {
      const { service, bucket } = makeService();
      const { key } = await service.put("org_test_1", png);

      expect(key.startsWith("org_test_1/")).toBe(true);
    ...
    it("rejects an unsupported content type", async () => {
      const { service, bucket } = makeService();
      await expect(
        service.put("org_test_1", { ...png, contentType: "text/plain" }),
    ...
    it("rejects a file over the size limit", async () => {
      const { service, bucket } = makeService();
      await expect(
        service.put("org_test_1", { ...png, size: 6 * 1024 * 1024 }),
      ).rejects.toBeInstanceOf(ValidationError);
      expect(bucket.put).not.toHaveBeenCalled();
    });
  });
